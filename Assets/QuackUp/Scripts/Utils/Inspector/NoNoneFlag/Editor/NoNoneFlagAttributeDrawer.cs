...
public class NoNoneFlagAttributeDrawer<T> : OdinAttributeDrawer<NoNoneFlagAttribute, T> where T : Enum
{
	...
	private void UpdateButtonContent()
	{
		m_buttonContent.text = !EditorGUI.showMixedValue
			? NoNoneFlagEnumSelector<T>.GetValueStringReflection(ValueEntry.SmartValue, out m_sdfIcon, out m_tooltip)
			: "-";
	...
	protected override void DrawPropertyLayout(GUIContent label)
	{
		...
		var selector = new NoNoneFlagEnumSelector<T>();
	...
	public class NoNoneFlagEnumSelector<TEnum> : EnumSelector<TEnum> where TEnum : Enum
	{
		...
		public override IEnumerable<TEnum> GetCurrentSelection()
		{
			...
			var newEnumSelected = (TEnum) Enum.ToObject(typeof (TEnum), (currentEnumInt));
			var minEnum = Enum.GetValues(typeof(TEnum)).Cast<TEnum>().Min();
			if (currentEnumInt == 0) newEnumSelected = minEnum;
